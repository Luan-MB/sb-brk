...
while1alocaMem: #compara valor apontado por aux com 0
    ...
    cmpq %rdx, %r9 #compara bestfit com 0
    jne elseif3alocaMem
    movq %rdi, %rdx #guarda o aux no bestfit
    jmp durwhile1alocaMem
elseif3alocaMem: #compara o tamanho de *(bestfit+1) com *(aux+1)
    pushq %rdx #salva rdx
    addq $8, %rdx
    movq (%rdx), %r11 #%r11 = *(bestfit+1)
    popq %rdx #restaura rdx
    cmpq %r10, %r11 #compara *(bestfit+1) com *(aux+1)
    jle durwhile1alocaMem
    movq %rdi, %rdx #bestfit = aux
...
poswhile1alocaMem: # após o while 1 no alocaMem
    cmpq %rdx, %r9
    je elseif5alocaMem
    movq $1, (%rdx) # *bestfit = 1
    addq $16, %rdx
    movq %rdx, %rax #%rax = *(bestfit+2)
    popq %rbp
    ret
